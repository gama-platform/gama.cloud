...
import msi.gama.lang.gaml.web.ui.views.toolbar.EditorToolbar;
...
public class GamlEditor extends AbstractGamlEditor implements IGamlBuilderListener, IGamlEditor,
...
//	IBoxDecorator decorator;
	...
	@Inject public IXtextResourceSetProvider resourceSetProvider;
	...
	private void updateToolbar(final GamlEditorState newState, final boolean forceState) {
		if (  !state.equals(newState)) {
			WorkbenchHelper.runInUI("Editor refresh", 50, (m) -> {
				...
				if (newState.showExperiments //&& !GamlFileExtension.isExperiment(getViewer().getDocument().getAdapter(IFile.class).getName())
						) {
...
//		}
	...
	public void insertText(final String s) {
		...
		try {
			new ReplaceEdit(offset, length, s).apply((org.eclipse.jface.text.IDocument)getViewer().getDocument());
		...
		getSelectionProvider().setSelection(new TextSelection(getViewer().getDocument(), offset + s.length(), 0));
	}
	...
	public String getSelectedText() {
		...
		if (length == 0) { return ""; }
		final IDocument doc = getViewer().getDocument();
		try {
			return doc.get(sel.getOffset(), length);
		} catch (final org.dslforge.styledtext.jface.BadLocationException e) {
			e.printStackTrace();
			return "";
		}
	}
...
//		getViewer().getDocument().readOnly(new CancelableUnitOfWork<Object, XtextResource>() {
//
//			@Override
