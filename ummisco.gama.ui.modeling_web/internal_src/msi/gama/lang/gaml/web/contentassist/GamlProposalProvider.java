			...
			if (cp != null) {
				...
				if (type != null) {
					if (type.equals("operator")) {
						isOperator = true;
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in operator) ")));
						cp.setImage(actionImage);
					} else if (type.equals("variable")) {
						
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in variable) ")));
						cp.setImage(varImage);
					} else if (type.equals("field")) {
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in field) ")));
						cp.setImage(varImage);
					} else if (type.equals("action")) {
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in action) ")));
						cp.setImage(actionImage);
					} else if (type.equals("unit")) {
						isOperator = true;
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in unit) ")));
						cp.setImage(null);
					} else if (type.equals("type")) {
						isOperator = true;
						cp.setDisplayString(new StyledString(cp.getDisplayString().concat(" (Built-in type) ")));
...
//		}
	...
	protected String getDisplayString(final EObject element, String qualifiedNameAsString, final String shortName) {
		...
		if (qualifiedNameAsString == null) {
			if (element != null) {
//				qualifiedNameAsString = provider.getText((ISyntacticElement) element);
			} else {
				return null;
			}
		}
