 ...
 *
 */
public class WebGui2 implements IGui {
	...
	public void runtimeError(final IScope scope, final GamaRuntimeException g) {
		if (g.isReported() || scope == null)
			return;
		if (GAMAWEB.getFrontmostController() != null && GAMA_WEB.getFrontmostController().isDisposing()) {
			return;
		}
	...
	public void prepareForExperiment(IScope scope, final IExperimentPlan exp) {
		...
		if (exp.isGui()) {
			...
			// initializer.run();
			// }
			// WorkbenchHelper.setWorkbenchWindowTitle(uid, exp.getName() + " - " +
...
//			icv.reset();
	...
	public void updateSpeedDisplay(IScope scope, final Double d, final boolean notify) {
		...
		if (speedStatus != null) {
			WorkbenchHelper.run(uid, () -> speedStatus.setInit(d, notify));
	...
	public void closeSimulationViews(IScope scope, final boolean openModelingPerspective, final boolean immediately) {
		final String uid = "admin";// RWT.getUISession().getAttribute("user").toString();
	...
	public String getExperimentState(String uid) {
		final IExperimentController controller = GAMAWEB.theControllers.get("admin");
		if (controller == null) {
			return NONE;
	...
	public void updateExperimentState(IScope scope, final String forcedState) {
		...
		// }
		// final String uid = WorkbenchHelper.UISession.get(scope);
		// final ISourceProviderService service = WorkbenchHelper.getService(uid,
		// ISourceProviderService.class);
		// final ISimulationStateProvider stateProvider = (ISimulationStateProvider)
		// service
	...
	public synchronized IStatusDisplayer getStatus(IScope scope) {
		...
		//
		// final String uid =
		// WorkbenchHelper.UISession.get(scope.getRoot().getExperiment().getSpecies().getExperimentScope());
	...
	// // final String uid=RWT.getUISession().getAttribute("user").toString();
	// final String uid =
	// WorkbenchHelper.UISession.get(scope.getExperiment().getSpecies().getExperimentScope());
	...
	public void applyLayout(IScope scope, Object layout, Boolean keepTabs, Boolean keepToolbars, boolean showEditors) {
		// final IDisplayLayoutManager manager =
		// WorkbenchHelper.getService(IDisplayLayoutManager.class);
		// if (manager != null) {
