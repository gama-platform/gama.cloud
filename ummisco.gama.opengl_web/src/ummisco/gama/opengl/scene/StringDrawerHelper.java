 ...
 *
 */
public class StringDrawerHelper {
	...
	private static final int GLU_TESS_WINDING_RULE = 0;
	private static final double GLU_TESS_WINDING_ODD = 0;
	private static final double GLU_TESS_WINDING_NONZERO = 0;
	...
	private class SideBuffer {
		...
		public void drawOn(final OpenGL openGL) {
			...
			for (int i = 0; i < currentIndex; i++) {
				gl.getGL2().glDrawArrays(GL2.GL_QUAD_STRIP, indices[i] / 3, (indices[i + 1] - indices[i]) / 3);
			}
			openGL.disable(GL2.GL_NORMAL_ARRAY);
		}
	}

	public class FaceBuffer extends GLUtessellatorCallbackAdapter {
