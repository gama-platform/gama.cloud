...
public class GamaPersistencyService implements IPersistencyService{
	...
	public static synchronized IPersistencyService getInstance() {
		if (INSTANCE == null) {
			INSTANCE = new GamaPersistencyService();
		}
		return INSTANCE;
	}
	...
	public String computeProjectName(IPath path) {
	...
	public String computeRelativePath(IPath path) {
		IPath relativePath = path.makeRelativeTo(rootPath);
		String[] segments = relativePath.segments();
		if (segments.length == 0) {
			throw new RuntimeException("Problem when computing relative path for " + path);
