...
public class WebOpenGL {
	...
	public WebOpenGL(final Abstract3DRenderer renderer) {
		glut = new GLUT();
...
//			t.destroy(gl); hqn88
	...
	public void setGL2(final WebGL2 gl2) {
		this.gl = gl2;
	}
...
//		texture.destroy(gl); hqn88
	...
	private static Texture buildTexture(final WebGL2 gl2, final File file) {
		try {
			final BufferedImage im = ImageUtils.getInstance().getImageFromFile(file, true);
			return buildTexture(gl2, im);
	...
	public static Texture buildTexture(final WebGL2 gl, final BufferedImage image) {
		return buildTexture(gl, image);
	}
