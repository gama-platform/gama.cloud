...
global
{
	...
	init
	{
		...
		list<agent> lstpredator0 <- Organism.Simple[0].get_predator();
		//the prey population of experiment 0 saved into the list lstprey0
		list<agent> lstprey0 <- Organism.Simple[0].get_prey();
		
		//the predator population of experiment 1 saved into the list lstpredator1
		list<agent> lstpredator1 <-Organism.Simple[1].get_predator();
		//the prey population of experiment 1 saved into the list lstprey1
		list<agent> lstprey1 <- Organism.Simple[1].get_prey();

		//the predator population of experiment 2 saved into the list lstpredator2
		list<agent> lstpredator2 <- Organism.Simple[2].get_predator();
		//the prey population of experiment 2 saved into the list lstprey2
		list<agent> lstprey2 <- Organism.Simple[2].get_prey();
		
		
		//mutate the popuplation of micro-model by assigning the list above to  the population of micro-models
		
		
		(Organism.Simple[0].simulation).lstPredator <- lstpredator2;
		(Organism.Simple[1].simulation).lstPredator <- lstprey2;
		(Organism.Simple[2].simulation).lstPredator <- lstprey1;
		
		
		(Organism.Simple[0].simulation).lstPrey <- lstprey0 + lstprey1;
		(Organism.Simple[1].simulation).lstPrey <- lstpredator1;
		(Organism.Simple[2].simulation).lstPrey <- lstpredator0 + lstprey2;
		
		//change the shape correspond with the new role of agent in the new populations
		ask (Organism.Simple accumulate each.simulation.lstPredator)
		{
			shape <- triangle(2);
		}

		ask (Organism.Simple accumulate each.simulation.lstPrey)
		{
			shape <- circle(0.5);
		}
